# 🚴 Bike Sales Dashboard – Excel Analysis

_Analyzing customer purchasing behavior and sales patterns to support better marketing and business decisions using Microsoft Excel._

---

## Table of Contents
- [Overview](#overview)
- [Project Objective](#project-objective)
- [Dataset](#dataset)
- [Tools & Technologies](#tools--technologies)
- [Dashboard Features](#dashboard-features)
- [Key Insights](#key-insights)
- [Dashboard](#dashboard)
- [Conclusion](#conclusion)
- [Role & Contribution](#role--contribution)
- [Author](#author)
---

## Overview
This project focuses on analyzing bike sales data to understand customer demographics, income levels, and commuting patterns that influence purchasing decisions. An interactive Excel dashboard was developed to convert raw customer data into clear and meaningful business insights.

---

## Project Objective
The primary goals of this project are:
- Identify which customer segments are more likely to purchase bikes  
- Analyze the impact of income, age, and commute distance on buying behavior  
- Provide data-driven insights to improve marketing and sales strategies  

---

## Dataset
🗂️ <a href="https://github.com/SHAKAR-AFZAL/Bike-Sales-Analysis-Excel-Project/blob/main/DATASET%20OF%20BIKE%20SALES.xlsx">Dataset View</a>  
 
 The Bike Sales dataset is stored in Excel format and includes the following customer attributes:
- Gender  
- Region  
- Education  
- Income  
- Age Bracket  
- Commute Distance  
- Purchase Status (Yes / No)

---

## Tools & Technologies
- **Microsoft Excel**
  - Pivot Tables  
  - Pivot Charts  
  - Slicers & Filters  
  - Data Cleaning & Formatting  
  - GitHub

---

## Dashboard Features

**Interactive Slicers**
- Gender  
- Region  
- Education  

**Key Visuals**
- Average Income per Purchase by Gender  
- Customer Commute Distance vs Purchased Bike  
- Customer Age Bracket Distribution  

---

## Key Insights

### Income & Purchase Behavior
- Customers who purchased bikes generally have **higher average incomes** than non-buyers.  
- Male buyers show **slightly higher income averages** compared to female buyers.  
- Purchases are most common among **middle-to-high income groups**.

### Commute Distance Impact
- Customers with **0–1 mile commute distance** have the **highest purchase rate**.  
- Bike purchases **decrease as commute distance increases**.  
- Commutes **above 10 miles** show the **lowest likelihood of bike purchases**.

### Age Bracket Trends
- The **Middle Age group (31–50 years)** represents the **largest buyer segment**.  
- Young adults and older age groups show comparatively **lower purchase counts**.

### Gender Distribution
- Male customers slightly lead in total purchases.  
- The difference is small, meaning **both genders are strong potential markets**.

### Education & Region Observations
- Higher education levels tend to align with **more stable purchasing behavior**.  
- Regional filters reveal **noticeable variations** in buying trends.

---

## Dashboard
<a href="https://github.com/SHAKAR-AFZAL/Bike-Sales-Analysis-Excel-Project/blob/main/DASHBOARD%20PHOTO.png" target="_blank">📊 Dashboard Picture</a>

The Excel dashboard displays:
- Income comparison by gender  
- Commute distance impact on purchases  
- Age-wise customer distribution  



---

## Conclusion
The analysis shows that **income level, commute distance, and age group** are the most influential factors in bike purchasing decisions. Middle-aged individuals with moderate to high incomes and shorter commute distances are the most likely customers.

Businesses can use these insights to:
- Target **working professionals** with focused campaigns  
- Promote bikes as **short-distance commuting solutions**  
- Offer **discounts or installment plans** for younger audiences  
- Apply **customer segmentation** for personalized marketing  

---

## Role & Contribution
- Data Cleaning & Preparation  
- Pivot Table and Chart Creation  
- Interactive Dashboard Design  
- Business Insight Generation  

---

## Author
**Mian Shakar Afzal**  

[LinkedIn](https://www.linkedin.com/in/mian-shakar-afzal-959b443a8/) | [GitHub](https://github.com/SHAKAR-AFZAL) | [Email](mailto:mianshakarafzal@gmail.com)



